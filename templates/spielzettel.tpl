<div class="container">
	<h2>
		Spielzettel<small class="float-right">Doppelkopfrunde am {$rundeDatum|date_format:"d.m.Y"} ({$rundeOrt})</small>
	</h2>
		...
		<table class="table table-hover">
			...
			<tbody>
				...
				<tr data-toggle="modal" data-target="#spielDetails" data-gameid="{$punkteliste.$i.gameId}">
					...
					<td class="pr-4 text-right {if isset($punkteliste.$i.$player_id.sieger)}table-success{/if}">{if
...
<div class="container">
	...
	<div class="table-responsive">
		<table class="table table-hover">
			<tr>
				<th class="col-1"></th> {foreach $players_round as $player_id => $player}
				<th class="col-1">{$player.anzeige|truncate:7:""}</th> {/foreach}
			</tr>
			{foreach $players_round as $player_id_row => $player}
			<tr>
				<th class="col-1">{$player.anzeige|truncate:7:""}</th> {foreach $players_round as $player_id_col => $player}
				<td class="col-1">{if
					isset($siegerpaar.$player_id_row.$player_id_col)}{$siegerpaar.$player_id_row.$player_id_col}{else}0{/if}</td>
				{/foreach}
			</tr>
			{/foreach}
		</table>
